...
import { cancelOrder, OrderApiError } from "@bep-nha-minh/api/services/admin-orders";
...
export async function POST(
  ...
  try {
    ...
    return successResponse({ order }, { message: "Đơn hàng đã được hủy." });
  } catch (error) {
    return handleMutationError(error, "Failed to cancel order");
  }
}
