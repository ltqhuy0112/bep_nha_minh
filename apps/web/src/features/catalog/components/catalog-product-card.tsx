  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-olive-700/10 bg-white shadow-soft">
      <div className="relative aspect-[4/3] overflow-hidden bg-beige-200">
        <Image src={presentation.image} alt={photoPending ? (locale === "vi" ? "Ảnh món cơm gà đang được cập nhật" : "Photo for the chicken rice meal is being updated") : presentation.alt} fill loading={eager ? "eager" : "lazy"} sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-105" />
      ...
      <div className="flex min-h-56 flex-col p-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-sm font-bold text-wood-500">{!orderingEnabled ? (locale === "vi" ? "Giá dự kiến: " : "Planned price: ") : ""}{formatPrice(product.unitPrice, product.currency, locale)}</p>
        ...
        <h2 className="mt-3 line-clamp-2 font-serif text-2xl leading-tight text-olive-900">{product.name}</h2>
        ...
        {orderingEnabled ? <div className="mt-4"><AddToCartButton slug={product.slug} locale={locale} disabled={!product.acceptingOrders || product.fulfillmentBlocked} /></div> : <Link href={`/${locale}#waitlist`} className="mt-4 inline-flex min-h-11 items-center text-sm font-bold text-olive-900 underline decoration-olive-500/50 underline-offset-4">{locale === "vi" ? "Báo tôi khi mở bán" : "Notify me when orders open"}</Link>}
      </div>
    </article>
  );
}
